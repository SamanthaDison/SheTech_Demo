...
export class Todo {
  ...
  get Template() {
    return ` <div class="d-flex justify-content-between align-items-center todo-item my-2">
                <i class=" fs-4 hoverable mdi mdi-checkbox-${this.completed ? 'marked' : 'blank-outline'}"
                  onclick="app.todosController.completeTodo('${this.id}')"></i>
                <p class="px-2 m-0 VT fs-3">${this.description}</p>
          <i class="fs-4 hoverable mdi mdi-alpha-x-box-outline" onclick="app.todosController.deleteTodo('${this.id}')"></i>
              </div>
        `
  }
}
